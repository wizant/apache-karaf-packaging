...
# Make source DEBs
$(BUILD_DIR)/%/.sdeb:
	@echo Make source DEBs
	-rm -rf $(PKG_BUILD_DIR)/deb/
	mkdir -p $(PKG_BUILD_DIR)/deb/$($(PKG)_PKG_NAME)-$(PKG_PKG_VERSION)
	# Only expands the tar if there is a source artifact
	echo $(PKG_BUILD_DIR)
	if [ -n "$($(PKG)_TARBALL_SRC)" ]; then \
	  cp $($(PKG)_DOWNLOAD_DST) $(PKG_BUILD_DIR)/deb/$($(PKG)_PKG_NAME)_$(PKG_PKG_VERSION).orig.tar.gz ;\
	  cd $(PKG_BUILD_DIR)/deb/$($(PKG)_PKG_NAME)-$(PKG_PKG_VERSION) && \
	    tar --strip-components 1 -xvf ../$($(PKG)_PKG_NAME)_$(PKG_PKG_VERSION).orig.tar.gz;\
	else \
		 tar -czf $(PKG_BUILD_DIR)/deb/$($(PKG)_PKG_NAME)_$(PKG_PKG_VERSION).orig.tar.gz LICENSE ;\
	fi
	cd $(PKG_BUILD_DIR)/deb/$($(PKG)_PKG_NAME)-$(PKG_PKG_VERSION) && \
          cp -r $(BASE_DIR)/packages/deb/$($(PKG)_NAME) debian && \
	  cp -r $(BASE_DIR)/packages/common/$($(PKG)_NAME)/* debian && \
	  echo -e "$(BIGTOP_BOM)" | tr ' ' '\012' >> debian/bigtop.bom && \
	  (echo -e "$($(PKG)_PKG_NAME) ($(PKG_PKG_VERSION)-$($(PKG)_RELEASE)) unstable; urgency=low\n" && \
